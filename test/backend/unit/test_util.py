...
def test_create_resource_with_role():
    with mock_aws():
        resource = create_resource_with_role("s3", DOCUMENT_STORAGE_BUCKET_READ_ROLE)
...
def test_create_resource_with_role_errors(aws_error_code, expected_error_class):
    with mock_aws():
        client = boto3.client("sts")
        stubber = Stubber(client)
        stubber.add_client_error("assume_role", service_error_code=aws_error_code)
        patcher = patch("boto3.client", return_value=client)
        with patcher, stubber, pytest.raises(expected_error_class):
            create_resource_with_role("s3", DOCUMENT_STORAGE_BUCKET_READ_ROLE)
