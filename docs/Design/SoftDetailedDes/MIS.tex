...
\begin{center}
  \begin{tabular}{p{3cm} p{4cm} p{3cm} p{3cm}}
    ...
    \textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions} \\
    \hline
    DBTable & \{TableName: string, Role: enum[``general'',
    ``admin''], Access: enum[``read'', ``write'']\} & DBTable &
    PermissionException \\
    \hline
    DBTable.get & \{Key: map[string {-\textgreater} any]\} &
    map[string {-\textgreater} any] & ItemNotFound, ExternalServiceFailure \\
    \hline
    DBTable.put & \{Item: map[string {-\textgreater} any], Condition:
    AttributeCondition\} & map[string {-\textgreater} any] &
    ConditionCheckFailed, ExternalServiceFailure, PermissionException \\
    \hline
    DBTable.delete & \{Key: map[string {-\textgreater} any],
    Condition: AttributeCondition\} & map[string {-\textgreater} any]
    & ExternalServiceFailure, ConditionCheckFailed, PermissionException \\
    \hline
    DBTable.query & \{KeyConditions: list[KeyCondition],
    AttributeConditions: list[AttributeCondition]\} & list[map[string
    {-\textgreater} any]] & ExternalServiceFailure \\
    \hline
  \end{tabular}
\end{center}
...
\begin{itemize}
  \item transition: $Database -> Database \cup \{i_{new}\}$, if $c ==
...
\begin{itemize}
  \item transition: $Database -> Database - \{i_{old}\}$, where
...
dbKey(Item: map[string -\textgreater any], TableName: string)
-\textgreater map[string -\textgreater any]:\\
\indent Returns the keys of the given db item, assuming it is from
the given table
