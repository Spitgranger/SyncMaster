...
Location Verification Module, Database Interaction Module, Routing Module
...
\begin{center}
  \begin{tabular}{>{\raggedright}p{4cm} >{\raggedright}p{3cm}
    ...
    \textbf{name:} string \newline \textbf{locationState:} boolean &
    ...
    location failed \\
    \hline
    validateToken & \textbf{authToken:} string  & boolean &
    \textbf{TokenInvalid:} Token invalid \\
    \hline
  \end{tabular}
\end{center}
...
\begin{center}
  \begin{tabular}{p{4cm} p{12cm}}
    ...
    \textbf{Name} & \textbf{Description} \\
    \hline
    issuedTokens & Table of tokens issued, can be represented as set
    of items $\{i_0, i_1, ..., i_n\}$, where $i_k:
    map[string \rightarrow  any], k\in[0,n]$ \\
    \hline
  \end{tabular}
\end{center}
...
\begin{itemize}
  \item transition: $issuedTokens \rightarrow issuedTokens \cup
    \{authToken\}$, if $(email \cap hashPassword) \in Database$,
    where $authToken$ is uniquely generated for the user and
    $hashPassword$ is the password after being hashed.
  \item output: $authToken$
  \item exception: InvalidCredentials
\end{itemize}

\noindent authenticateContractor(email: email, name: name, locationState: bool):
\begin{itemize}
  \item transition: $issuedTokens \rightarrow issuedTokens \cup
    \{authToken\}$, if $(email \cap name) \in Database \wedge bool ==
    true$, where $authToken$ is uniquely generated for the user.
  \item output: $authToken$
  \item exception: InvalidCredentials, LocationVerificationFailed
\end{itemize}

\noindent validateToken(authToken: token):
\begin{itemize}
  \item transition: N/A
  \item output: true if $token \in issuedTokens$, else false.
  \item exception: TokenInvalid
\end{itemize}
...
\begin{center}
  \begin{tabular}{>{\raggedright}p{3cm} >{\raggedright}p{4cm}
    ...
    \textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Description} \\
    \hline
    generateToken & \textbf{userID:} string & \textbf{authToken:} string &
    Generates unique secure token for authenticated user \\
    \hline
    hashPassword & \textbf{password:} string & \textbf{hashPassword:} string &
    Hashes password to securely check against database \\
    \hline
  \end{tabular}
\end{center}
