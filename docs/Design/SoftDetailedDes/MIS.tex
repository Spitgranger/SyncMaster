...
\begin{center}
  \begin{tabular}{>{\raggedright}p{3cm} >{\raggedright}p{5cm} >{\raggedright}p{3cm} p{4cm}}
    \hline
    \textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions} \\
    \hline
    DBTable & TableName: string \newline Role: enum[``general'',
    ``admin''] \newline Access: enum[``read'', ``write''] & DBTable &
    PermissionException \\
    \hline
    DBTable.get & Key: map[string $\rightarrow$ any] &
    map[string $\rightarrow$ any] & ItemNotFound \newline ExternalServiceFailure \\
    \hline
    DBTable.put & Item: map[string $\rightarrow$ any] \newline Condition:
    AttributeCondition & map[string $\rightarrow$ any] &
    ConditionCheckFailed \newline ExternalServiceFailure \newline PermissionException \\
    \hline
    DBTable.delete & Key: map[string $\rightarrow$ any] \newline
    Condition: AttributeCondition & map[string $\rightarrow$ any]
    & ExternalServiceFailure \newline ConditionCheckFailed \newline PermissionException \\
    \hline
    DBTable.query & KeyConditions: list[KeyCondition] \newline
    AttributeConditions: list[AttributeCondition] & list[map[string $\rightarrow$ any]] & ExternalServiceFailure \\
    \hline
  \end{tabular}
\end{center}
...
\begin{center}
  \begin{tabular}{>{\raggedright}p{2cm} >{\raggedright}p{5cm} >{\raggedright}p{3.5cm} p{4.5cm}}
    \hline
    \textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Description} \\
    \hline
    dbKey & Item: map[string $\rightarrow$ any] \newline TableName: string & map[string $\rightarrow$ any] & Returns the keys of the given db item, assuming it is from the given table \\
    \hline
  \end{tabular}
\end{center}
