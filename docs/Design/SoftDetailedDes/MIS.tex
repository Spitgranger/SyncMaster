...
\begin{itemize}
  \item transition: $Database \rightarrow Database \cup
    ...
    \{userID, email, hashPassword, \ldots\}$
  \item output: $userID$, where userID is uniquely generated for the email.
...
\noindent deleteUserUser(userID: userID):
\begin{itemize}
...
\begin{center}
  \begin{tabular}{>{\raggedright}p{3cm} >{\raggedright}p{4cm}
    ...
    \hline
    hashPassword & \textbf{password:} string & \textbf{authToken:} string &
    Hashes password for secure storage and verification in database.\\
    \hline
  \end{tabular}
\end{center}
