...
See SRS Documentation at \url{https://github.com/Spitgranger/SyncMaster/blob/main/docs/SRS-Volere/SRS.pdf}.
...
SyncMaster, a facilities management application for the Technical Services team at the Water Division of the City of Hamilton.
This application enables the Technical Services team to effectively distribute
water station documentation to stakeholders as a single source of truth. It also acts as an authentication tool for external
...
\begin{center}
  \begin{tabular}{p{3cm} p{4cm} p{2cm} p{3cm}}
    ...
    addWorkOrder & \textbf{workOrder:}int & boolean & illegalArgument: Work Order not valid number \\
    addDescription & \textbf{description:}string & boolean & None \\
    uploadFile & \textbf{file}: any file type & boolean & FileSizeLimit: file exceeds accepted size\\
    \hline
  \end{tabular}
\end{center}
...
\begin{center}
  \begin{tabular}{p{4cm} p{12cm}}
    ...
    \textbf{Name} & \textbf{Description} \\
    \hline
    jobTypes & Set of available jobs $\{j_0, j_1, ..., j_n\}$, where $j_k:string, k\in[0,n]$ \\
    files & Set of uploaded files $\{f_0, f_1, ..., f_m\}$, where $f_k:file, k\in[0,m]$ \\
...
\begin{itemize}
  \item transition: jobType := jobType $\in$ jobType, the set of possible job types.
