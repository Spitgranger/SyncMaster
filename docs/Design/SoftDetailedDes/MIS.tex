...
\begin{center}
  \begin{tabular}{>{\raggedright}p{2.5cm} >{\raggedright}p{3.5cm}
    ...
    \textbf{password:} string & \textbf{userID:} string &
    \textbf{DuplicateUser:} Existing email address used \\
    \hline
    editUser & \textbf{userID:} string \newline
    \textbf{edit:} map[string $\rightarrow$ any] & boolean & \textbf{UserNotFound:}
    userID not in database \\
    \hline
    deleteUser & \textbf{userID:} string & boolean & \textbf{UserNotFound:}
    userID not in database \\
    \hline
    getUser & \textbf{userID:} string & map[string $\rightarrow$ any] & \textbf{UserNotFound:}
    userID not in database \\
    \hline
  \end{tabular}
\end{center}
...
\begin{center}
  \begin{tabular}{p{4cm} p{12cm}}
    ...
    \textbf{Name} & \textbf{Description} \\
    \hline
    Database & Set of registered users, can be represented as set of items $\{i_0, i_1, ..., i_n\}$, where $i_k:
    map[string \rightarrow  any], k\in[0,n]$ \\
    \hline
  \end{tabular}
\end{center}
...
\noindent createUserUser(email: email, password: password):
\begin{itemize}
  \item transition: $Database \rightarrow Database \cup
    \{User_new\}$, if $(email \cap password) \in Database $, where
    $authToken$ is uniquely generated for the user.
