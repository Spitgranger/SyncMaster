...
\begin{tabularx}{\textwidth}{p{3cm}p{2cm}X}
  ...
  3/10/2025 & 1.1 & Remove Job Management Module\\
  \bottomrule
\end{tabularx}
...
% MIS of Job Management Module Beginning
\section{\sout{MIS of Job Management Module}} \label{Module}

\subsection{\sout{Module}}

\sout{Module for contractors to enter their job information.}

\subsection{\sout{Uses}}
\sout{Database Interaction Module}
...
\begin{center}
  \begin{tabular}{>{\raggedright}p{2cm} >{\raggedright}p{5cm}
    ...
    \hline
    \sout{\textbf{Name}} & \sout{\textbf{In}} & \sout{\textbf{Out}} & \sout{\textbf{Exceptions}} \\
    \hline
    \sout{createJob} & \sout{\textbf{jobType:} string, \textbf{workOrder:} int,
    \textbf{userID:} string, \textbf{description:} string} &
    \sout{\textbf{jobID:} string} &  \sout{\textbf{ExternalServiceFailure:} An
    internal error from AWS} \newline
    \sout{\textbf{DuplicateWorkOrder:} Job with same work order already in database} \\
    \hline
    \sout{editJob} & \sout{\textbf{jobID:} string, \textbf{workOrder:} int,
    \textbf{description:} string, \textbf{jobType:} string} & \sout{None} &
    \sout{\textbf{ExternalServiceFailure:} An internal error from AWS} \newline
    \sout{\textbf{JobAlreadyCompleted:} This job is already closed}\\
    \hline
    \sout{completeJob} & \sout{\textbf{jobID:} string} & \sout{None} &
    \sout{\textbf{ExternalServiceFailure:} An internal error from AWS}\\
    \hline
    \sout{deleteJob} & \sout{\textbf{jobID:} string} & \sout{None} &
    \sout{\textbf{ExternalServiceFailure:} An internal error from AWS} \newline
    \sout{\textbf{JobAlreadyCompleted:} This job is already closed}\\
    \hline
  \end{tabular}
...
\begin{center}
  \begin{tabular}{p{4cm} p{12cm}}
    \hline
    \sout{\textbf{Name}} & \sout{\textbf{Description}} \\
    \hline
    \sout{jobTypes} & \sout{Set of available jobs $\{j_0, j_1, ..., j_n\}$, where
    $j_k:string, k\in[0,n]$} \\
    \sout{jobType} & \sout{string $\in$ jobTypes} \\
    \sout{jobID} & \sout{string identifying a created job} \\
    \sout{workOrder} & \sout{number of type int} \\
    \sout{description} & \sout{work description of type string} \\
    \sout{status} & \sout{status of job, value in $\{open, closed$\}}\\
    \hline
  \end{tabular}
\end{center}
...
\begin{center}
  \begin{tabular}{p{6cm} p{10cm}}
    ...
    \sout{\textbf{Name}} & \sout{\textbf{Description}} \\
    \hline
    \sout{LAMBDA\_EXECUTION\_ROLE} & \sout{When an AWS Lambda Function (The chosen
    AWS compute service for this project), it has an AWS IAM role
    attached to it, that it uses when running. This role needs
    permission for job database access for this module to work} \\
    \hline
  \end{tabular}
\end{center}
...
\sout{LAMBDA\_EXECUTION\_ROLE has the required permissions in AWS to
execute the lambda's required tasks.}

\subsubsection{\sout{Access Routine Semantics}}

\noindent \sout{createJob(\textbf{jobType:}string, \textbf{workOrder:}int,
\textbf{userID:}string, \textbf{description:}string):}
\begin{itemize}
  \item \sout{transition: creates job associated with userID}
  \item \sout{output: the jobID}
  \item \sout{exception:  ExternalServiceFailure, DuplicateWorkOrder}
\end{itemize}

\noindent \sout{editJob(\textbf{jobID:}string, \textbf{workOrder}int,
\textbf{description}string, \textbf{jobType}string):}
\begin{itemize}
  \item \sout{transition: edit job attributes if not closed}
  \item \sout{exception:  ExternalServiceFailure, JobAlreadyCompleted}
\end{itemize}

\noindent \sout{completeJob(\textbf{jobID:}string):}
\begin{itemize}
  \item \sout{transition: status = closed}
  \item \sout{exception:  ExternalServiceFailure}
\end{itemize}

\noindent \sout{deleteJob(\textbf{jobID:}string):}
\begin{itemize}
  \item \sout{transition: remove job if status is open}
  \item \sout{exception:  ExternalServiceFailure, JobAlreadyCompleted}
\end{itemize}
...
\sout{N/A}
% MIS of Job Management Module Ending
