...
\subsubsection{Database Interaction Module}
...
\begin{enumerate}
  ...
  \item{UT-DB4\\}
    ...
    Input: Put method called, but and unknown error occurs in AWS
  ...
  \item{UT-DB9\\}
    ...
    Output: Item still exists, error is raised indicating conditon check failed
  ...
  \item{UT-DB12\\}
    ...
    Test Case Derivation: If the update method is called you would epect the requested changes to show up in the database
  ...
  \item{UT-DB16\\}
    ...
    Test Case Derivation: When AWS fails, no changes should be made, and an error should be raised to callers can handle it appropriately
