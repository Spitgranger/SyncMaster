...
\begin{enumerate}
  ...
  \item{TC-LF-1\\}
    ...
    user interface and finds that they all follow a similar colour
    palette to existing applications used by the city of Hamilton.
  ...
  \item{TC-LF-2\\}
    ...
    Input: User logs in, and attempts all core functions for
    their role while using devices with common desktop screen
    resolutions (1920x1080, 1366x768, 1280x1024, 1024x768), common
    mobile device screen resolutions (375x667, 414x736, 360x800,
    390x884), and common tablet screen resolutions (768x1024,
    1024x768, 601x962). Alongside this for each screen resolution,
    tests should be replicated on both Google Chrome and Microsoft Edge.

    Output: User is able to interact with the system on all screen
    sizes and understand the interface regardless of screen size or browser

    How test will be performed: One user who works at the city of
    Hamilton for each type of role (General/Contractor, Manager,
    Admin), will be guided through the core functionalities important
    for their roles by a member of the development team and asked to
    evaluate how easy it is to perform the core functionalities on a
    scale of 1-10. This test will be repeated of all screen sizes
    mentioned in the inputs. If all users provide a score $\ge$ 5 out
    of 10, for all screen sizes and all browsers, then the test is
