...
const AcknowledgmentForm: React.FC<AcknowledgmentFormProps> = ({
  documents = [],
  onProceed,
}) => {
  ...
  const [acknowledged, setAcknowledged] = useState<Record<string, boolean>>(
    sortedDocuments.reduce(
      (acc, doc) => ({ ...acc, [doc.document_id]: false }),
      {}
    )
  );

  const [trainingCompleted, setTrainingCompleted] = useState<string | null>(
    null
  );
  const [accompaniedByEmployee, setAccompaniedByEmployee] = useState<
    string | null
  >(null);
  ...
  const handleCityEmployeeIdChange = (
    event: React.ChangeEvent<HTMLInputElement>
  ) => {
        ...
        sortedDocuments.map((doc) => (
          <Box key={doc.document_id} sx={{ my: 2, textAlign: 'left' }}>
            ...
            <FormControlLabel
              control={
                <Checkbox
                  checked={acknowledged[doc.document_id]}
      ...
      <Box textAlign={'left'}>
        <FormControl component="fieldset" sx={{ mt: 2, textAlign: 'left' }}>
          <FormLabel component="legend">
            <Typography variant="body2" color="black">
              Have you completed the Annual H&S Training?
            </Typography>
          </FormLabel>
        ...
        {trainingCompleted === 'no' && (
          <FormControl component="fieldset" sx={{ mt: 2, textAlign: 'left' }}>
            <FormLabel component="legend">
              <Typography variant="body2" color="black">
                Are you accompanied by a City Employee?
              </Typography>
            </FormLabel>
            <RadioGroup
              value={accompaniedByEmployee}
            ...
            {accompaniedByEmployee === 'yes' && (
              <>
                <Typography
                  ...
                  sx={{ mt: 2 }}
                >
                  Please provide the ID of the city employee accompanying you.{' '}
