    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(getSiteVisitDetails.fulfilled, (state, action) => {
                state.work_order = action.payload.work_order !== undefined ? action.payload.work_order : state.work_order;
                state.description = action.payload.description !== undefined ? action.payload.description : state.description;
...
export const enterSite = createAsyncThunk(
    ...
    async ({ site_id, idToken, allowed_tracking, ack_status, on_site, employee_id }:
        {
            site_id: string | null, idToken: string | null, allowed_tracking: boolean,
...
export const uploadAttachment = createAsyncThunk(
    ...
    async (uploadData: any) => {
        ...
        const s3PresignedURLResponse = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/protected/documents/get_presigned_url/${s3_key}`, {
            method: "GET",
            headers: {
                "Authorization": `${idToken}`,
            }
        ...
        const s3FileUploadResponse = await fetch(s3PresignedURLResponsejson["s3_presigned_url"]["url"], {
            method: "POST",
            body: formData,
        });
        ...
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/protected/site/${site_id}/visit/${entryTime}/attachments/add`, {
            method: 'PATCH',
            headers: {
                "Content-Type": 'application/json',
                "Authorization": `${idToken}`,
...
export const removeAttachment = createAsyncThunk(
    ...
    async (removeData: any) => {
        ...
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/protected/site/${site_id}/visit/${entryTime}/attachments/remove`, {
            method: 'PATCH',
            headers: {
                "Content-Type": 'application/json',
                "Authorization": `${idToken}`,
...
export const exitSite = createAsyncThunk(
    'site/exitSite',
    async ({ site_id, idToken, entryTime }: { site_id: string | null, idToken: string | null, entryTime: string | null }) => {
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/protected/site/${site_id}/exit/${entryTime}`, {
            method: 'PATCH',
            headers: {
                "Authorization": `${idToken}`,
            }
...
export const getSiteVisitDetails = createAsyncThunk(
    ...
    async ({ site_id, idToken, entryTime }: { site_id: string | null, idToken: string | null, entryTime: string | null }) => {
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/protected/site/${site_id}/visit/${entryTime}`, {
            method: 'GET',
            headers: {
                "Authorization": `${idToken}`,
            }
...
export const updateWorkOrderNumber = createAsyncThunk(
    ...
    async ({ site_id, idToken, entryTime, work_order }: { site_id: string | null, idToken: string | null, entryTime: string | null, work_order: number }) => {
        const requestBody = {
...
export const updateWorkDescription = createAsyncThunk(
    ...
    async ({ site_id, idToken, entryTime, description }: { site_id: string | null, idToken: string | null, entryTime: string | null, description: string }) => {
        const requestBody = {
