...
export const getAcknowledgementDocuments = createAsyncThunk(
  ...
  async (pathData: any) => {
    ...
    const generalRequestPath = `protected/documents/ALL/${folder_id}/get_files`;
    const siteSpecificRequestPath = `protected/documents/${site_id}/${folder_id}/get_files`;
    ...
    const siteSpecificLayer1Documents = await fetchDocuments(
      siteSpecificRequestPath
    );
