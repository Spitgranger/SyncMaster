...
const ejfakle = () => {
    ...
    const currentRoot = decodeURI(router.asPath.replace("/documents/", ""))

    const breadcrumbPath = convertPathToArray(router.asPath.replace("/documents/", ""))
    ...
    const deleteItem = async (pathname) => {
        try {
            const deleteResponse = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/documents/delete?file_path=${encodeURIComponent(pathname)}&site_id=ALL`, {
                method: "DELETE",
            });
    ...
    const handleDoubleClick = (itemId, isFolder) => {
        ...
        if (isFolder) {
            router.push(`/documents/${state["folders"][itemId]["path"]}`); // Navigate to the folder
        } else {
                        ...
                        <TableBody>
                            {
                                ...
                                state["folders"][currentRoot]["childFolders"].map(function (folderId: string) {
                                    return (
                                        <TableRow
                                            key={folderId}
                                            onClick={() => handleDoubleClick(folderId, true)}
                            ...
                            }
                            {
                                ...
                                state["folders"][currentRoot]["childFiles"].map(function (fileId: string) {
                                    return (
                                        <TableRow
                                            key={fileId}
                                            onClick={() => handleDoubleClick(fileId)}
...
//     return <DocumentTreeContextProvider>{page}</DocumentTreeContextProvider>;
// }
